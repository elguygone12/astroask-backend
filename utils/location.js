...
const InputScreen = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const offsetSec = data.results[0].annotations.timezone.offset_sec;

      // Convert to +HH:MM format
      const offsetHours = offsetSec / 3600;
      const sign = offsetHours >= 0 ? '+' : '-';
      const absHours = Math.floor(Math.abs(offsetHours));
      const absMinutes = Math.round((Math.abs(offsetHours) - absHours) * 60);
      const timezone = `${sign}${absHours.toString().padStart(2, '0')}:${absMinutes.toString().padStart(2, '0')}`;
